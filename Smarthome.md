...
Connect the ESP32 GPIOs to the Relay IN1 and IN2 pins.

Crucial Safety Step: Remove the VCC-JDVCC jumper on the relay module.

Connect your ESP32 GND to the Relay GND.

Connect a separate 5V power source to the Relay JD-VCC and GND.

2. Blynk Cloud Configuration

Create a new Template in the Blynk Web Dashboard.

Configure Datastreams as per the Pin Mapping table above.

Set up the Mobile App dashboard with buttons (Switch mode) and a Time Input widget.

3. Firmware Deployment

Open Blynk_Project_Final.ino in the Arduino IDE.

Ensure you have the ESP32 board manager installed.

Input your unique BLYNK_AUTH_TOKEN, SSID, and PASSWORD.

Select "DOIT ESP32 DEVKIT V1" and hit Upload.
